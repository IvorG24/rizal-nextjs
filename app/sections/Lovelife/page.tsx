...
const LovelifeSection = () => {
  ...
  return (
    <section className="min-h-screen h-full w-full flex flex-col items-center justify-center bg-white text-black" id="LovelifeSection">
        ...
        <Accordion variant="shadow" className="bg-gray-800 text-white">
          {accordionItems.map(({ name, key }, index) => (
            <AccordionItem
              ...
              title={
                <div className={`${openIndex === index ? 'text-left' : 'text-center'}`}>
                  {openIndex === index && (
                    <div className="flex items-center mt-2">
                      <FontAwesomeIcon icon={faCircleDot} className="mr-1 text-base" />
                      <span className="text-base">{index + 1}</span>
                    </div>
                  )}
                  {!openIndex || openIndex === index ? name : name} {/* Ensure the title is displayed when the accordion is open or closed */}
                </div>
              }
              indicator={
              ...
              className={`transition-all duration-300 ease-in-out ${
                openIndex === index ? 'w-64 transform rotate-90 py-4 ml-6' : 'w-64'
              }`}
            >
