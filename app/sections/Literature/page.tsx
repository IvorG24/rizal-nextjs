...
function Literature() {
  ...
  return (
    <section
      id="literature"
      ref={ref}
      className="min-h-screen h-full w-full relative text-white p-10 flex flex-col justify-around gap-y-20"
      style={{
    ...
    >
      ...
      <h1 className="text-center text-2xl lg:text-[80px] text-yellow-500 relative z-20">
        ...
        {LiteratureData.map((item, index) => (
          <div key={index} className="relative">
            ...
            <div
              ...
              className="absolute top-0 left-0 w-full  flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity duration-300 bg-black bg-opacity-80 text-4xl rounded-lg font-bold h-full z-50"
            >
              Explore More
            </div>
            <div className="absolute inset-0 flex items-center justify-center text-center">
              <h1 className="text-white text-2xl lg:text-2xl w-72 z-40">
                {item.title}
              </h1>
            </div>
          </div>
